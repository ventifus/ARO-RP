...
func injectClusterManager(c *cosmosdb.FakeClusterManagerConfigurationDocumentClient) {
	...
	c.SetConflictChecker(clusterManagerConflictChecker)
}

func clusterManagerConflictChecker(one *api.ClusterManagerConfigurationDocument, two *api.ClusterManagerConfigurationDocument) bool {
	if one.ID != "" && two.ID != "" && one.ID == two.ID {
		return true
	}
	if one.PartitionKey != "" && two.PartitionKey != "" && one.PartitionKey == two.PartitionKey {
		return true
	}
	return false
}
